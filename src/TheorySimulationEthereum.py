...
class Simulation:
    ...
    def __init__(self, simulation_type='ethereum_protocol', MAX_OUTBOUND_CONNECTIONS=13, with_evil_nodes=False,
                 ...
                 outbound_distribution='const13_25', data={}):
    ...
    def run(self, t_start=1, t_end=60, n_iterations=124, plot_first_x_graphs=10,
            ...
            MAX_OUTBOUND_CONNECTIONS=13, numb_nodes=3000, p_a=0.95):
        self.MAX_OUTBOUND_CONNECTIONS = MAX_OUTBOUND_CONNECTIONS
        ...
        while self.MAX_OUTBOUND_CONNECTIONS != len(self.DG.node[node_id][self.simulation_protocol].outbound):
            ...
            for envelope in envelopes:
                ...
                if envelope['receiver'] not in self.DG.nodes:
                    if envelope['receiver'] in self.DG.node[node_id][self.simulation_protocol].record_outgoing_conn['RandomNodes']:
                        self.DG.node[node_id][self.simulation_protocol].record_outgoing_conn['RandomNodes'].remove(envelope['receiver'])
                    if envelope['receiver'] in self.DG.node[node_id][self.simulation_protocol].record_outgoing_conn['lookupBuf']:
                        self.DG.node[node_id][self.simulation_protocol].record_outgoing_conn['lookupBuf'].remove(envelope['receiver'])
    ...
    def _new_node_connects_to_network(self, show_network=False):
        ...
        if self.simulation_protocol == 'ethereum_protocol':
            ...
            self.DG.node[self.DG_last_id][self.simulation_protocol] = en.TheoryEthereumNode(self.DG_last_id,
                                                                                      self.node_id_to_rnd,
                                                                                     self.simulation_time,
                                                                                     self.FIXED_DNS,
                                                                                     self.DG,
    ...
    def _get_addresses_from_neighbour(self, node_id, envelope=None, show_protocol=False):
        if envelope is None:
            ...
            envelope_list = self.DG.node[node_id][self.simulation_protocol].ask_neighbour_to_get_addresses(
                self.simulation_time)
    ...
    def _initialize_fixed_dns_servers(self):
        ...
        for ii in self.FIXED_DNS:
            ...
            self.DG.add_node(ii)
            if self.simulation_protocol == 'ethereum_protocol':
                self.DG.node[ii][self.simulation_protocol] = en.TheoryEthereumNode(ii, self.node_id_to_rnd, self.simulation_time, self.FIXED_DNS,
    ...
    def _initial_node_discovery(self, node_id):
        if self.DG.node[node_id][self.simulation_protocol]._hard_coded_dns is True:
            return
        envelope_list = self.DG.node[node_id][self.simulation_protocol].ask_node_for_self_lookup(self.simulation_time)
        for envelope_1 in envelope_list:
            if envelope_1['receiver'] not in self.DG.nodes:
                continue
            envelope_2 = self.DG.node[envelope_1['receiver']][self.simulation_protocol].receive_message(
