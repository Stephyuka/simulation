...
class TheoryMoneroNode(MoneroConstant):
    id: int
    ############################
    # initialization
    ############################
    ...
    def receive_message(self, t, envelope):
        ...
        if envelope['connect_as_outbound'] == 'rejected':
            if envelope['sender'] in self.white_list:
                self.white_list.remove(envelope['sender'])
            if envelope['sender'] in self.grey_list:
                self.grey_list.remove(envelope['sender'])
    ...
    def _get_address_to_connect(self):
        ...
        if conn_count < (self.MAX_OUTBOUND_CONNECTIONS + len(_intersection_of_2_lists(self.outbound.keys(), self._hard_coded_dns))):
            ...
            if conn_count < expected_white_connections:
                ...
                if len(self.anchor_list) - len(_intersection_of_2_lists(self.outbound.keys(), self.anchor_list)) != 0:
                    #anchor
                        return random.choice(tuple(set(self.anchor_list) - _intersection_of_2_lists(self.anchor_list, self.outbound.keys())))
                elif len(set(self.white_list)) > 0 and len(set(self.white_list) - _intersection_of_2_lists(self.white_list, self.outbound.keys())) != 0:
                    return random.choice(tuple(set(self.white_list) - _intersection_of_2_lists(self.white_list, self.outbound.keys())))
                elif len(self.grey_list) > 0:
                    return random.choice(tuple(set(self.grey_list) - set(self.outbound.keys())))
                else:
                    choose = set(self.my_DNS) - set(self.outbound.keys())
                    return random.choice(list(choose)) if len(choose) > 0 else None

            else:
                #try grey and then white
                if len(self.grey_list) > 0 and len(set(self.grey_list) - set(self.outbound.keys())) > 0:
                    return random.choice(tuple(set(self.grey_list) - set(self.outbound.keys())))
                elif len(set(self.white_list) - set(self.outbound.keys())) > 0:
                    return random.choice(tuple(set(self.white_list) - set(_intersection_of_2_lists(self.white_list, self.outbound.keys()))))
                else:
                    choose = set(self.my_DNS) - set(self.outbound.keys())
                    return random.choice(list(choose)) if len(choose) > 0 else None
